...
> git submodule 挂载于其 `data/` 目录。语料校验事实源 = Rust crate
> **tianquan**（主仓 `crates/tianquan`，`tianquan-check`）。

## 目录

```
data/tang/300-poems.jsonl      唐诗三百首（蘅塘退士编，通行本，321 首）
...
cargo run -q -p tianquan --bin tianquan-check -- ../wenshuge-data/data/tang/300-poems.jsonl
```
